...
- `internal/webui` — 内置只读 Web UI（`/ui/`，静态资源 go:embed 进 binary，发布包自包含；顶层配置 `webui: false` 可整体不挂载，重启生效）：templ 服务端渲染 + 会话 cookie（admin token 登录；cookie 仅对 GET admin API 生效）；`views/` templ 源与生成码，`static/` embed 的 CSS/JS（app.js 片段轮询、logs.js NDJSON 日志流）
