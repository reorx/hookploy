...
- **httpapi `admin` 中间件扩展**：`Bearer 头 OR 有效 session cookie` 皆可通过，使 JS 同源 fetch 现有 JSON 端点时 cookie 自动生效。cookie 路径仅接受 **GET/HEAD** 请求（v1 UI 只读；POST 类端点仍必须 Bearer，天然免 CSRF）。
...
（2026-07-22 结构评审定稿：站点结构、进行中卡片密度、服务页合并拓扑图、详情页时间线联动，见本节各处。）

**导航模型**：无导航栏。`/ui/` 单页堆叠三区块，服务详情、部署详情为钻取页；顶栏 logo 点击回 `/ui/`，钻取页顶部显示面包屑（`echo_server` / `echo_server / dp_19a8f2`）。将来服务规模增长时，区块标题可加「查看全部 →」演化出独立列表页——不是一次性锁死的决定。
...
- 进行中卡片内嵌一个小日志窗口（~8 行高），直接连该 deploy 的 follow 流（logs.js 复用）；卡片模式只保留最近 200 行、只追尾不回滚，防止长部署撑爆 DOM。完整日志去详情页看
- 除日志窗口外整页按片段轮询刷新（见 §6.2）

### 5.2 `/ui/services/{name}` 服务详情

- 头部：服务名、image、webhook 开关、timeout
- **Rollout × 实例拓扑图**（合并原「实例表」与「波次图」两个区块，消除实例名重复出现）：波次为横向列（`wave 1 → wave 2 → …`），列内每个实例一张小卡：实例名、服务器（含在线状态点）、dir。一张图同时回答「部署在哪、按什么顺序、哪些并行」。templ 实现即双层循环
...
- **时间线 → 日志联动**：op 行可点击 → 自动设置下方日志区的 execution 过滤，并滚动到该 op 的起始锚点。锚点由**前端合成**：日志帧自带 `execution_id`/`op_index`（`api.LogLine`），渲染时给每个 `(execution_id, op_index)` 分组的首帧标锚点 id（`log-<execution_id>-<op_index>`）。注意引擎并不在每个 op 边界发 system 帧（`OpStart` 只写 op 记录表），不能依赖 system 帧做锚点；零日志 op（如 `env.require`）点击时仅设过滤、不滚动。~15 行 JS
- **日志查看器**（页面主体，终端风深色区）：
  - 终态 deploy：一次性拉 `?format=json` 回放渲染
  - 运行中：连 `?follow=1` 流，实时追加；收到 done 帧后停流并刷新上方状态区
  - 行前缀：`[instance/op名]`，stderr 行着橙红色，system 流（healthcheck、重试提示等 op 内部消息）着灰色
...
两种挂载模式：**详情页模式**（全量渲染 + op 锚点 + execution 过滤 + 固定底部开关）与**卡片模式**（dashboard 进行中卡片：仅追尾、最多保留 200 行、无过滤）。锚点在渲染时由帧的 `execution_id`/`op_index` 前端合成（每分组首帧），见 §5.3。
...
2. **API：`GET /deploys` + `GET /services/{name}`** — 先写 httpapi 测试（含 404、limit 上限、ServiceDetail 字段完整性、ops 线格式一致性）→ 实现 → 更新 `docs/json-output.md`；CLI golden 仅当同时新增 `hookploy service <name>` CLI 命令时才需更新（该命令可选、不阻塞 UI）
...
预计新增代码量：Go ~800 行（含测试）、templ ~400 行、CSS ~300 行、JS ~190 行。
...
- **多实例日志交织**：v1 用行前缀 + execution 过滤 + 时间线联动跳转解决；若真机体验差，v2 备选 GitHub Actions 式「日志内嵌 execution 折叠块」——日志帧已带 execution_id，按帧分发到多容器即可，不是单行道。取舍：折叠式会失去跨实例时序对照。
